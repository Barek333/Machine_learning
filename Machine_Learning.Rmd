...
```{r,tidy=TRUE}
training1 <- training[sample(nrow(training),1000,replace=F),]
training_val <- training[sample(nrow(training),1000,replace=F),]
...
But in the end I decided on using a boosting model because of the large number of variables. I did a 25-fold cross-validation and got an accuracy of 0.886. If I used a bigger amount of training data and more crossvalidation I would get a better accuracy as well.

```{r,tidy=TRUE}
set.seed(33333)
#modelFit <-train(classe ~.,data=training2,method="rf",prox=TRUE)
